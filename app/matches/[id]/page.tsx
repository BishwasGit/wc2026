...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL || ""}/api/matches/${id}`);
        ...
        <div className="flex items-center justify-center gap-4 md:gap-8">
          ...
          <div className="min-w-[100px] text-center">
            {isFinished || isLive ? (
              ...
              <div className="font-mono text-2xl md:text-4xl text-gray-500">
                vs
              </div>
            )}
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          {match.venue && (
            <>
              ...
              <span className="text-white text-right">{match.venue}</span>
            </>
          )}
          {match.stage && (
            <>
              <span className="text-gray-500">Stage</span>
              <span className="text-white text-right">{match.stage}</span>
            </>
          )}
          {match.group && (
            <>
              <span className="text-gray-500">Group</span>
              <span className="text-white text-right">{match.group}</span>
            </>
          )}
          {match.matchday != null && (
            <>
              <span className="text-gray-500">Matchday</span>
              <span className="text-white text-right">{match.matchday}</span>
            </>
          )}
        </div>
      </div>
      ...
      <div className="card p-5 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            Predict the final score for this match
          </p>
        </div>
        <Link
          href="/predict"
...
function LoadingSkeleton() {
  return (
    <div className="space-y-6">
      ...
      <div className="card p-6">
        <div className="flex justify-center mb-6">
          <div className="h-5 w-16 bg-[#1a2e1a] rounded animate-pulse" />
        </div>
...
async function MatchFetcher({ id }: { id: string }) {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL || ""}/api/matches/${id}`,
    { next: { revalidate: 60 } }
  );
